...
jest.mock("@expo/vector-icons", () => {
  ...
  return {
    Ionicons: ({ name, ...props }: { name: string }) => <Text {...props}>{name}</Text>,
  };
});

describe("SearchBar", () => {
  it("renders text input with placeholder", () => {
    render(<SearchBar value="" onChangeText={jest.fn()} onClear={jest.fn()} />);
  ...
  it("shows clear button and calls onClear when pressed", () => {
    ...
    render(<SearchBar value="test" onChangeText={jest.fn()} onClear={onClear} />);
